	...
	modifyButton := widget.NewButtonWithIcon("Modify Torrent", theme.DocumentSaveIcon(), func() {
		...
		go func() {
			...
			if presetConfig != nil && presetSelect.Selected != "" && presetSelect.Selected != "Manual" {
				...
				// Apply NoDate from preset
				if opts.NoDate != nil && *opts.NoDate {
					t.CreationDate = 0
					modified = true
				}

				// Apply NoCreator from preset
			...
			// Apply UI settings (overriding preset if applicable)
			...
			// Always set source (empty string will clear it)
			info := t.GetInfo()
			...
			// Always set comment (empty string will clear it)
			...
			// Private flag
			isPrivate := privateCheck.Checked
			info = t.GetInfo() // Refresh info since it may have been modified above
			...
			// Handle NoDate option
			if noDateCheck.Checked && t.CreationDate != 0 {
				t.CreationDate = 0
				modified = true
			}

			// Handle NoCreator option
